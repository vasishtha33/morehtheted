...
<template>
  <header class="sticky top-0 z-50 bg-[#0a0514]/90 backdrop-blur-md border-b border-amber-400/30 shadow-lg">
    <div class="container mx-auto px-4 py-4 flex justify-between items-center">
      
      <!-- Логотип / Название -->
      <router-link to="/" class="text-2xl md:text-3xl font-serif text-amber-400 tracking-wider drop-shadow-[0_0_8px_rgba(251,191,36,0.5)] hover:scale-105 transition-transform duration-300">
        Moreh Tedeh
      </router-link>
      ...
      <!-- Кнопка гамбургера (только на мобильных и планшетах) -->
      <button @click="toggleMenu" class="lg:hidden text-amber-400 focus:outline-none z-50">
        <svg v-if="!isMenuOpen" xmlns="http://www.w3.org/2000/svg" class="h-7 w-7" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
...
.nav-link {
  @apply relative text-gray-300 hover:text-amber-400 transition-colors duration-300 after:absolute after:bottom-[-4px] after:left-0 after:w-0 after:h-0.5 after:bg-amber-400 after:transition-all after:duration-300 hover:after:w-full;
}
...
/* Активная ссылка (можно добавить класс router-link-active) */
.router-link-active {
